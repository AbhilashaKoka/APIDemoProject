...
public class PracticalFormPage {
  ...
  static WebDriver  webDriver;
    ...
    public void SubmitPracticeForm() {
        AbstractComponent.scrollWindowUp(Frame_formFrames);
        Frame_formFrames.click();
        PracticeForms.click();
        AbstractComponent.scrollWindowUp(InputBox_FirstName);
        ...
        AbstractComponent.scrollWindowUp(RadioButton_Male);

        if(!RadioButton_Male.isSelected()){
            AbstractComponent.MoveToElement(RadioButton_Male);
            RadioButton_Male.sendKeys(Keys.TAB);
        }
        AbstractComponent.scrollWindowUp(Inputbox_Mobile);

        Inputbox_Mobile.sendKeys("90249234234", Keys.TAB);
        AbstractComponent.scrollWindowUp(Inputbox_DateOfBirth);

        Inputbox_DateOfBirth.sendKeys("25 JAN 2024", Keys.TAB, Keys.ENTER, Keys.TAB);
        AbstractComponent.scrollWindowUp(Inputbox_Subject);

        Inputbox_Subject.sendKeys("Computer Science",Keys.TAB, Keys.TAB);
        AbstractComponent.scrollWindowUp(CheckBox_Hobbies);

        if(!CheckBox_Hobbies.isSelected()) {
            AbstractComponent.MoveToElement(CheckBox_Hobbies);
            CheckBox_Hobbies.sendKeys(Keys.TAB, Keys.TAB);
        }

        Upload_Picture.sendKeys("D:\\Users\\akoka\\Downloads\\sampleFile.jpeg");
        AbstractComponent.scrollWindowUp(TextArea_CurrentAdress);
